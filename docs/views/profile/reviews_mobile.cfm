...
<div class="content print-area">
	<div class="welcome-box">
		...
		<cfif successMessage neq "">
			<h3 style="color:black;" id="review-success-message"><cfoutput>#successMessage#</cfoutput></h3>
		</cfif>
		...
		<h2 class="add-a-review"><a href="#" class="comment-link" id="comment-open">Add a Review</a></h2>
...
<!--- 		<div class="comment-modal hidefirst">
			...
			<div class="add-comment" id="addcomment" style="display:none;">
				<h2 class="add-a-review"><a href="#" class="comment-link" id="comment-close">Add a Review</a></h2>
				<cfoutput>
				...
				<form name="addcomment" class="FBpop" action="#params.silourl###review-success-message" method="post">
					<fieldset>
						...
						<div>
							<div class="row">
								...
								<div class="styled-input"><input name="firstname" id="firstname" type="text" class="noPreText" value="#params.firstname#" /></div>
							</div>
							<div class="row">
								<label <cfif ListFind(errorList,"lastname")>class="invalid-input" </cfif>for="lastname">Last Name</label>
								<div class="styled-input"><input name="lastname" id="lastname" type="text" class="noPreText" value="#params.lastname#" /></div>
							</div>
							...
							<div class="row">
								...
								<div class="styled-input"><input name="city" id="city" type="text" class="noPreText" value="#params.city#" /></div>
							</div>
							<div class="row">
								<label <cfif ListFind(errorList,"state")>class="invalid-input" </cfif>for="state">State</label>
									<div class="styled-select">
									<select name="state" id="state" class="comment-states">
							...
							<div class="row">
								...
								<div class="styled-input"><input name="email" id="email" type="text" class="noPreText" value="#params.email#" /></div>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="procedures-checklist">
								...
								<select name="procedures">
									<option value="">Select Procedure #x#</option>
									<cfloop query="variables.procedures">
										<option value="#variables.procedures.id#">#variables.procedures.name#</option>
									</cfloop>
								</select>
								</cfloop>
							</div>
						...
						<div class="row">
							...
							<div class="rating-container">
								...
								<div class="rating-scale">
									<div class="rating-0">Unlikely</div>
									<div class="rating-10">Likely</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<label class="full<cfif ListFind(errorList,"captcha")> invalid-input</cfif>">Please enter the numbers pictured below:</label>
							<div class="styled-input captcha-input"><input type="text" class="noPreText" name="captcha_input"></div>
						...
						#Request.oUser.getCheckBox(labelStyle="")#
					</fieldset>
			...
			<cfloop query="recommendations">
				<div class="video-holder comment page_#Ceiling(currentrow/5)#<cfif currentrow gt 5> hidden</cfif>" itemprop="review" itemscope itemtype="http://schema.org/Review">
					...
					<cfif recommendations.photoLocation neq "">
						<td class="review-photo">
							<script language="Javascript">
									var checkPhoto = $.ajax({
											url: "#recommendations.photoLocation#",
											type: "GET"
										});

									checkPhoto.done(function(data){
										//console.log("data:" + data);
									});

									checkPhoto.success(function(data, textStatus, jqXHR){
										var responseMimeType = jqXHR.getResponseHeader("content-type");
										var re = /^image/i;
										//console.log(jqXHR.getResponseHeader("content-type"));
										//console.log(responseMimeType.search(re));
										if(responseMimeType.search(re) == -1)
										{
											//console.log("Not an image");
											//document.getElementById("recommendationPhoto").style.display = "none";
											 removeReviewFacebookImage();

										}
									});

									checkPhoto.fail(function( jqXHR, textStatus ) {
									  //console.log( "Request failed: " + jqXHR );
									  // Facebook image does not exist or returned an error. remove
									  removeReviewFacebookImage();
									  //document.getElementById("recommendationPhoto").style.display = "none";
									});

									function removeReviewFacebookImage()	{
										$("[src='#recommendations.photoLocation#']").hide();
									}

							</script>

							<img src="#recommendations.photoLocation#" style="float:left; margin-right:15px;" id="recommendationPhoto">
						</td>
					</cfif>
					...
					<p align="right">
						...
						<span class="author-city-state">#city#, #state#</span>
					</p>
				</div>
			</cfloop>
			#IncludePartial("commentpagination")#
