...
<cfoutput>
	...
	#javaScriptIncludeTag(source="profile/main_mobile", head=true)#
</cfoutput>
...
<cfoutput>
	<div class="topsections">
		<div class="photo">
			<!--- <img src="/images/profile/doctor_photo_border.png" id="doctor_photo_border"> --->
			<img class="profile-doctor-photo" src="#doctorPhoto#" width="214" height="195" alt="#doctor.firstName# #doctor.middleName# #doctor.lastName#<cfif doctor.title neq "">, #doctor.title#</cfif>" id="doctor_photo" />
			<cfif topDoc.recordCount and topDoc.showTopDocSeal OR isAdvisoryBoard>
			<div class="ProfileLogos">
			<cfif topDoc.recordCount and topDoc.showTopDocSeal>
				<div id="badge-top-doctor" class="tooltip" title="TOP DOCTOR | To qualify as a Top Doctor, a practice must have met a high engagement level with contributions to LocateADoc.com, have a majority of positive comments and reviews, and have a medical license and/or training in the specialty listed.">
					Top Doctor
					<!--- <img src="/images/layout/badges/topdoctor_current_large.png" id="top_doc_logo"> --->
				</div>
			</cfif>
			<cfif isAdvisoryBoard>
				<div id="badge-board-member" class="tooltip" title="ADVISORY BOARD MEMBER | The LocateADoc Advisory Board is compiled of a small group of elite medical industry professionals involved in performing various treatments and procedures patients search for on LocateADoc.com. The Board provides beneficial industry insights to the LocateADoc team, honest feedback and constructive criticism on current and new patient needs and overall business direction as it relates to industry trends. The Board and Executive Management continuously explore ways to build trust and ensure potential patients have the best user experience, every time, when searching for a doctor on LocateADoc.com.">
					Board Member
		...
		<div class="vcard">
			<div class="holder">
				<div class="frame">
					<p class="fn name" itemprop="name">#doctor.fullNameWithTitle#</p>
					<p id="practicename">#practice.name#</p>

					<div id="get-a-consultation">
						<cfif hasContactForm>
							<a href="<cfif params.action neq "contact">/#doctor.siloName#/contact</cfif>##ContactForm" class="native-anchor" data-ajax="false" rel="external" style="text-decoration: none; color: white;">Get a Consultation</a>
						<cfelse>
							<a href="##contact-a-doctor" style="text-decoration: none; color: white;" data-ajax="false" rel="external">Get a Consultation</a>
						</cfif>
					</div>
					...
					<cfif ((not isPastAdvertiser and not isBasicPlusOver2Leads and not isBasic)
						...
						<div class="contact-list">
							<cfif currentLocationDetails.phonePlus neq "">
								<!--- <img src="/images/layout/profile-icon-phone.png" alt="phone" border="0"> ---><span class="contact-phone-number" itemprop="telephone"><a tabIndex="-1" href="tel:#currentLocationDetails.phonePlus#">Call #formatPhone(currentLocationDetails.phonePlus)#</a><br /></span>
							<cfelseif isYext and (currentLocationDetails.phoneYext neq "")>
								<!--- <img src="/images/layout/profile-icon-phone.png" alt="phone" border="0"> ---><span class="contact-phone-number" itemprop="telephone"><a tabIndex="-1" href="tel:#currentLocationDetails.phoneYext#">Call #formatPhone(currentLocationDetails.phoneYext)#</a><br /></span>
							<cfelseif currentLocation.phone NEQ "">
								<a href="##" onclick="profileMiniOpen(3); return false;"><!--- <img src="/images/layout/profile-icon-phone.png" alt="phone" border="0"> --->Get Phone Number</a><br />
							</cfif>

							<cfif hasContactForm>
								<!--- Has to be on one line otherwise formatting breaks --->
								<cfif coupons.recordCount>
									<span><a href="#URLfor(controller=doctor.siloName,action="offers")#"><!--- <img src="/images/layout/profile-icon-special-offers.png" alt="offers" border="0"> --->Special Offers</a></span>
								</cfif>
									<span><a href="<cfif params.action neq "contact">/#doctor.siloName#/contact</cfif>##ContactForm" class="native-anchor" data-ajax="false" rel="external">Email Doctor</a>
										<br />
									</span>
								...
								<cfif trim(validURL) NEQ "">
									...
									<cfif isAdvertiser>
										<a	href="#websiteURL#"
											...
											clicktracksection="ContactInformation" target="#validURL#"><!--- <img src="/images/layout/profile-icon-website.png" alt="website" border="0" rel="nofollow"> --->Doctor's Website</a><br />
									<cfelse>
										<a href="##" onclick="profileMiniOpen(2); return false;"><!--- <img src="/images/layout/profile-icon-website.png" alt="website" border="0"> --->Doctor's Website</a><br />
									</cfif>
									</span>
								</cfif>
							</cfif>
